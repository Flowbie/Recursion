...
public class AllRecursive {
    ...
    */
   public static int menu(int choice) throws InputMismatchException{
      Scanner scan = new Scanner(System.in);
      if (choice != 1 && choice != 2 && choice != 3 && choice != 0) {    // Validate user input to 1, 2, 3, or 0
         System.out.println("\nPlease press 1, 2, or 3 and enter or 0 to quit");  // Will loop after each function 
         try {
            choice = scan.nextInt();
            menu(choice);
         } catch (InputMismatchException as) {                 // If user throws IME error set choice to -1 and rerun menu
         ...
         if (choice == 1) {                                    // Run levels and invTriangle function
            System.out.print("\nThis function can print an inverted triangle \nHow many levels do you want to print? \nType a number between 1 and 12 inclusively: ");
            int iLevels = 0;
            try {
               iLevels = scan.nextInt();
            } catch (InputMismatchException as) {              // If user enters invalid input rerun menu
               System.out.println("\nInvalid Entry: This function only works with numbers");
               return menu(choice);
            }
            if (iLevels < 1 || iLevels > 12) {                 // Validate iLevels between 1-12 inclusively or reset menu
               System.out.println("\nInvalid Entry: The server is connected to a potato please keep numbers between 1 and 12 inclusively");
               menu(choice);
            ...
            System.out.print("\nThis function will take a number, print every number in descending order, and stop at 0 \nType a number between 1 and 12 inclusively: ");
            int iNum = 0;
            try {
               iNum = scan.nextInt();
            } catch (InputMismatchException as) {              // If user enters invalid input rerun menu
            ...
            if (iNum < 1 || iNum > 12) {                       // Validate iNum 1-12 inclusively or reset menu
               System.out.println("\nInvalid Entry: The server is connected to a potato please keep numbers between 1 and 12 inclusively");
               menu(choice);
            } else {
            ...
            System.out.print("\nThis function will take a number and add all the numbers between 1 and your number \nType a number between 1 and 12 inclusively: ");
            int iNum = 0;
            try {
               iNum = scan.nextInt();
            } catch (InputMismatchException as) {              // If user enters invalid input rerun menu
